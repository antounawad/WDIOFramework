...
{
    internal class NoService : IUpdater
    {
        private const int FILE_OPERATION_TIMEOUT = 5000;
        private const int FILE_OPERATION_WAIT_TIME = 250;
        ...
        public void FileCopy(string sourceFileName, string destFileName, bool overwrite)
        {
            try
            {
                ...
                while (stopwatch.ElapsedMilliseconds < FILE_OPERATION_TIMEOUT)
                {
                    try
                    {
                    ...
                    Thread.Sleep(FILE_OPERATION_WAIT_TIME);
                }
                throw ex;
            }
            catch (Exception exception)
        ...
        public void ProcessStart(WorkerProcess processStartInfo, bool wait)
        {
            try
            {
                using (var process = new Process { StartInfo = processStartInfo.ToProcessStartInfo() })
                {
                    process.Start();
                    if (wait)
                    {
